...
function init3DCarousel() {
    ...
    function updateCarousel(direction) {
        ...
        // Get current items array (DOM order changes)
        const currentItems = Array.from(carousel.querySelectorAll('.carousel-item'));
        ...
        if (direction === 'next') {
            ...
            setTimeout(() => {
                // Rearrange items - move first to end
                const firstItem = currentItems[0];
                carousel.appendChild(firstItem);
                carousel.classList.remove('rotate-right');
                
                // Get updated items array
                const updatedItems = Array.from(carousel.querySelectorAll('.carousel-item'));
                
                // Set new active
                updatedItems[0].classList.add('active');
            ...
            setTimeout(() => {
                // Rearrange items - move last to beginning
                const lastItem = currentItems[currentItems.length - 1];
                carousel.insertBefore(lastItem, currentItems[0]);
